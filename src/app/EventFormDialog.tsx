...
import {
  createEvenstEndPoint,
  deleteEvenstEndPoint,
  ICalendar,
  IEditingEvent,
  updateEvenstEndPoint,
} from "./backend";
...
export function EventFormDialog(props: IEventFormDialogProps) {
  ...
  function save(evt: React.FormEvent) {
    ...
    if (event) {
      if (validate()) {
        if (isNew) {
          createEvenstEndPoint(event).then(props.onSave);
        } else {
          updateEvenstEndPoint(event).then(props.onSave);
        }
      }
    }
  }
  ...
  function deleteEvent() {
    if (event) {
      deleteEvenstEndPoint(event.id!).then(props.onSave);
    }
  }
